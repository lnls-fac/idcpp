...
void DELTA::gen_delta(const Block& genblock, const unsigned int nr_periods, const double vertical_gap, const double horizontal_gap, const double block_separation){
  this->cs.gen_halbach_cassette(genblock,  Matrix3D<>::rotx90n, nr_periods, block_separation);
  this->ci.gen_halbach_cassette(genblock,  Matrix3D<>::rotx90n, nr_periods, block_separation);
  this->ce.gen_halbach_cassette(genblock,  Matrix3D<>::rotx90p, nr_periods, block_separation);
  this->cd.gen_halbach_cassette(genblock,  Matrix3D<>::rotx90p, nr_periods, block_separation);

  Vector3D<> dim = genblock.get_dim();

  this->cs.set_x(0.0);
  this->cs.set_z(+(vertical_gap + dim[2])/2.0);
  this->cs.set_ycenter(0.0);

  this->ci.set_x(0.0);
  this->ci.set_z(-(vertical_gap + dim[2])/2.0);
  this->ci.set_ycenter(0.0);

  this->cd.set_x(+(horizontal_gap + dim[0])/2.0);
  this->cd.set_z(0.0);
  this->cd.set_ycenter(0.0);

  this->ce.set_x(-(horizontal_gap + dim[0])/2.0);
  this->ce.set_z(0.0);
...
template <typename T>
Vector3D<T> DELTA::get_field(const Vector3D<T>& pos) const {
  Vector3D<T> field;
